...
async function seed(bcryptSalt: Salt) {
  ...
  const queryFunctionAddUser =
    'create or replace function public.handle_new_user() \n    returns trigger \n    language plpgsql \n    security definer set search_path = public \n    as $$ \n    begin \n    IF (new.raw_app_meta_data->>\'provider\' = \'email\') then \n     insert into public."User" (id,"createdAt","updatedAt",username,password,roles) \n     values (new.id::text,new.created_at,new.updated_at,new.email,new.encrypted_password, ARRAY[new.role]) \n    ON CONFLICT (id) \n    DO \n      UPDATE SET "createdAt" = new.created_at,"updatedAt" = new.updated_at,username = new.email,password = new.encrypted_password,roles = ARRAY[new.role]; \n    ELSE  \n      IF new.role = \'authenticated\'  \n      THEN\n        UPDATE auth.users SET role=\'user\' WHERE id = new.id;\n        new.role := \'user\';\n        RETURN NEW;\n      END IF;\n      \n      insert into public."User" (id,"createdAt","updatedAt","firstName",username,password,roles) \n      values (new.id::text,new.created_at,new.updated_at,new.raw_user_meta_data->>\'name\',new.email,new.encrypted_password,ARRAY[new.role]) \n      ON CONFLICT (id) \n      DO UPDATE SET "createdAt" = new.created_at,"updatedAt" = new.updated_at, "firstName" = new.raw_user_meta_data->>\'name\', username = new.email, roles = ARRAY[new.role],password=new.encrypted_password; \n\n    END IF;\n    return new; \n    end; \n    $$; ';
