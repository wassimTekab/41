  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="appconfig">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(appconfig)"
            :key="index"
          >
...
const { getAppConfigById } = useAppConfigStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getAppConfigById(id);
  setCurrentPageBreadcrumbs("appconfigs", ["detail"]);
});
</script>
<style>
.bottom {
